...
st.markdown("""
     ...
     <p> Predict <b> Tip Amount </b> from <b> Toatl Bill</b> using linear regression...
     </div>      
...
adju_r2 = 1-(1-r2)*(len(y_test)-1)/(len(y_test)-2)

# visualization

st.markdown('<div class="card">', unsafe_allow_html=True)
st.subheader("Toatal Bill vs Tip Amount")
...
c3,c4 = st.columns(2)
c3.metric("R-Squared error", f"{r2:.2f}")
c4.metric("Adjusted R-Squared error", f"{adju_r2:.2f}")
st.markdown('</div>', unsafe_allow_html=True)
...
st.markdown('<div class="card">', unsafe_allow_html=True)
st.subheader("Predicct Tip Amount")
